...
치과 홈페이지 URL을 넣으면 **생성형 AI(ChatGPT·Perplexity)가 지금 그 병원을 추천하고 있는지** 실제로 물어보고 증거로 보여주는 웹앱. 메디앤메디 영업 리드젠 도구.
...
| 심층 경쟁사 비교 + 개선 계획표 | `POST /api/compare` | operator 전용 |
...
Operator 기능은 `x-operator-key` 헤더 필요 → 없으면 401.
...
│   ├── compare.js                POST → 경쟁사 심층 비교 (operator)
...
│   ├── auth-check.js             GET  → operator-key 검증
│   └── unlock.js                 POST → 이메일 unlock
│
├── lib/                          서버 공유 라이브러리
...
│
├── test/                         Node.js 내장 test runner (256 tests)
...
### Operator 게이트
`x-operator-key` 헤더 = `process.env.OPERATOR_KEY` 환경변수. 불일치 → 401.
경쟁사 실명·비교 데이터는 이 게이트 뒤에서만 노출.
...
OPERATOR_KEY          운영자 모드 키 (x-operator-key 헤더)
