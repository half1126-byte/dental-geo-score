...
## naver-ai.html (디자인 리뷰 2회차 이월, 2026-07-15)

- [ ] **계약치과 비식별 스크린샷 확보·교체** — 예약/완결성/리뷰 viz의 보도자료 이미지(와인바·동물병원·카페)를 실제 계약치과 플레이스 화면(병원명 블러)으로 교체. 원장 동의 선행. *왜:* 원장 페르소나의 "내 얘기다" 동일시가 타업종 이미지에서 깨짐 — 페이지 유일의 치과 실증(ChatGPT 인용)은 텍스트뿐. *의존:* 거래처 동의 + 캡처 SOP.
- [ ] **진단 발송 SLA 문구 확정** — 성공 메시지 "정리해 보내드립니다"에 운영 확인 후 "영업일 N일 내" 명시. *왜:* 기한 없는 약속은 리드 신뢰를 흘림, 단 못 지킬 기한은 역효과라 운영 처리속도 확인 선행. 코드 1줄.
- [ ] **개인정보처리방침 실링크** — 푸터 방침 링크가 회사 홈으로 연결됨. PII 수집 폼이 있는 페이지라 실제 방침 URL 필요(medinmedi.com 내 확보 또는 신설). *왜:* 법적 안전장치 + 신뢰.
- [ ] **날짜 종속 클레임 월간 재검증** — "2026-07 기준 광고불가"·"연내 40%"·"AI탭 1,000만" 등 만료성 주장을 월간 관측 리포트 루틴에 "페이지 클레임 재검증" 항목으로 편입. *왜:* 기획안의 정책변동 추적 원칙과 정합 — 오래된 주장은 신뢰 전체를 무너뜨림.
