 ...
 * @returns {{status:number, contentType:string, body:string, finalUrl:string, ip:string, redirectChain:string[], headers:object}}
 */
export async function safeFetch(input, options = {}) {
  ...
  for (let hop = 0; hop <= opts.maxRedirects; hop++) {
    ...
    // headers exposed so callers can read framing policy (x-frame-options / CSP frame-ancestors)
    // for iframe-embeddability decisions. Other callers ignore the extra field.
    return { status, contentType, body, finalUrl: url.href, ip: pinned.address, redirectChain: chain, headers };
  }
