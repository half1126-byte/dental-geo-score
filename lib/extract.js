...
export function extractJsonLd($) {
  ...
  function walk(node) {
    ...
    if (node.sameAs && !hasSameAsAuthority) {
      const urls = Array.isArray(node.sameAs) ? node.sameAs : [node.sameAs];
      if (urls.some((u) => typeof u === 'string' && SAMEAS_AUTHORITY_RE.test(u))) hasSameAsAuthority = true;
    }
    if (node.address) {
      hasAddress = true;
  ...
  return {
    ...
    hasSameAsAuthority,
    hasAggregateRating: typeList.some((t) => /AggregateRating/i.test(t)),
