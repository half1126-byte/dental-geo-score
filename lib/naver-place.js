...
// Iteratively strip institution-type suffixes to isolate the brand token.
// "트리움치과의원" → "트리움", "시그니처치과교정과치과의원" → "시그니처",
// "둔촌동 트리움치과" → "둔촌동트리움". Guarded: never strips below 2 chars.
const NAME_SUFFIXES = /(치과의원|치과병원|교정과|한의원|클리닉|덴탈|치과|병원|의원)$/;

export function coreClinicName(raw) {
  ...
  do {
    prev = core;
    core = core.replace(NAME_SUFFIXES, '');
  } while (core !== prev && core.length >= 2);
  return core.length >= 2 ? core : norm;
}

/**
 * Try to find the target clinic in a list of normalized places.
 * Matching priority: exact phone > phone last-7 > exact core name > core name
 * includes (both directions). The bidirectional core match is what lets a
 * title-derived guess like "둔촌동 트리움치과" find the place "트리움치과의원" —
 * one-way substring failed there and the panel contradicted its own
 * competitor list. Returns null if no match found.
 */
export function findTargetClinic(items, { clinicPhone, clinicName } = {}) {
  ...
  // Name match on stripped brand cores — exact equality wins over fuzzy includes.
  const targetCore = coreClinicName(clinicName);
  if (targetCore.length >= 2) {
    const cores = items.map((it) => coreClinicName(it.name));
    const exact = items.find((_, i) => cores[i].length >= 2 && cores[i] === targetCore);
    if (exact) return exact;
    const fuzzy = items.find((_, i) =>
      cores[i].length >= 2 && (cores[i].includes(targetCore) || targetCore.includes(cores[i]))
    );
