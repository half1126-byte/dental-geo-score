// Per-engine measurement + optimization config, derived from docs/engines/ (live-verified 2026-07-07: 3엔진 실호출 스모크 — gpt-4o·claude-sonnet-4-6·sonar 전부 인용 검출).
...
export const ENGINES = {
  ...
  claude: {
    ...
    measureMode: 'auto-api',
    api: '/v1/messages + web_search_20250305 (direct-call + tool_choice 강제)',
    ...
    caveats: ['현재 사용 버전: web_search_20260209 (tool_choice 강제)', '$10/1k·150자 캡은 라이브 재확인 전 단정 금지', '측정 모델 기본 claude-sonnet-4-6 — ANTHROPIC_SEARCH_MODEL env로 오버라이드'],
  },
  perplexity: {
...
// Extended conversational set shared by the API's region-aware query selection.
// Keep byte-identical with public/query-preview.js; query-preview.test.js guards drift.
export function allQueryVariants({ district = '', procedure = '', businessType = '치과' } = {}) {
  ...
  const conversational = [
    ...
    hasD && hasP && `${d}역 근처 ${p} ${bt} 아무데나 예약해도 괜찮을까?`,
...
export function buildPrompts({ district = '', procedure = '', businessType = '치과' } = {}) {
  ...
  const users = allQueryVariants({ district: d, procedure: p, businessType: bt }).slice(0, 4);
