...
import { sameRegistrableDomain } from './normalize.js';
...
export const ENGINES = {
  ...
  claude: {
    ...
    caveats: ['web_search_20260318 은 환각 → 사용 금지', '$10/1k·150자 캡은 라이브 재확인 전 단정 금지'],
  },
  perplexity: {
...
export function buildPrompts({ district = '', procedure = '' } = {}) {
  const d = district.trim();
  const p = procedure.trim();
  const system = `You are helping a patient in ${d || '한국'} find a dental clinic. Use web search.`;
  const users = [
...
export function isClinicCited(engine, response, clinicDomain) {
  ...
  return {
    ...
    allCitedDomains: [...new Set(urls.map((u) => { try { return new URL(u).hostname; } catch { return u; } }))],
