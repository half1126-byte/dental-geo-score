...
export function buildOpenAIRequest({ system, user }, loc = {}) {
  ...
  return {
    model: loc.model || process.env.OPENAI_SEARCH_MODEL || 'gpt-4o', // override via OPENAI_SEARCH_MODEL; gpt-4o-search-preview deprecates 2026-07-23
...
export async function runCitationPanel({ clinicDomain, region = '', procedure = '', keys = {}, loc = {}, repeats = 1, concurrency = 6, nowIso = null, fetchImpl = fetch }) {
  ...
  const outcomes = await mapLimit(tasks, concurrency, async ({ engine, p }) => {
    try {
      ...
      return { engine, p, hit: isClinicCited(engine, resp, clinicDomain) };
    } catch (e) {
