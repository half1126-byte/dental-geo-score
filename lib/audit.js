...
export async function auditUrl(inputUrl, opts = {}) {
  ...
  return {
    ...
    ...scored,
    // 구조 점수를 '무엇을 해야 하는가'로 번역한다. 실측 패널(panel)이 함께 오면
    // 실제 인용 결과까지 반영해 원인을 좁힌다 — 여기서는 구조 단독 진단.
    diagnosis: diagnose({ scored: { ...scored, needsHeadless: signals.needsHeadless }, panel: null }),
    agentActionability,
