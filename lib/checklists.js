...
// Per-client provisioning state for the 공신력 측정 스택 (POC-derived 2026-06-23).
// auto-connector = we pull via API once the credential is provisioned; manual = operator keys it in.
export const CLIENT_SETUP_CHECKLIST = {
  title: '거래처 측정 스택 셋업',
  note: '자동 커넥터 2개(ga4-ai·cf-aibots) + 수동 3개(GSC·GBP·네이버). 자동도 거래처별 크레덴셜 1스텝 필요.',
  items: [
    { id: 'ga4-sa', mode: 'auto-connector', label: 'GA4: 에이전시 서비스계정 이메일을 속성에 Viewer로 추가 + propertyId 등록', field: 'ga4PropertyId' },
    { id: 'cf-token', mode: 'auto-connector', label: 'Cloudflare: Account Analytics:Read API 토큰 발급 + zoneTag, 플랜 티어 기록(precise=Enterprise/UA=Free·Pro)', field: 'cfZoneTag,cfPlan' },
    { id: 'gsc-rollout', mode: 'manual', label: 'GSC: Gen-AI 리포트 롤아웃 대상 속성인지 확인 → 운영자가 AI 노출수 수기 입력(API 없음)', field: 'gscAiImpressions' },
    { id: 'gbp-manager', mode: 'manual', label: 'GBP(Phase 3): 에이전시를 매니저로 등록 + 프로젝트 API 승인(0→300 QPM) — 완료 시 자동 전환', field: 'gbpLocationId' },
    { id: 'naver-login', mode: 'manual', label: '네이버: 서치어드바이저 + 스마트플레이스 로그인 인계 → 운영자가 ~90일 export 수기 입력(API 없음)', field: 'naverManual' },
    { id: 'utm-chatgpt', mode: 'manual', label: 'OAI-SearchBot 허용 시 ChatGPT 유입이 utm_source=chatgpt.com으로 GA4에서 확인 가능(하한선 지표) — lib/connectors/ga4-ai.js(medium=ai-assistant)와 연결', field: 'utmChatgptConfirmed' },
  ],
};
