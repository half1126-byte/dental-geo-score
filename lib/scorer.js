...
// never grades/낙제 (의료광고법 + 전환). The score is a NECESSARY-condition hygiene check — it
// does NOT predict real citation (자사 검증 v0.1: 점수↔실제 인용 무상관).
...
export function scorePage({ robots = {}, signals = {} }) {
  ...
  // 1. AI 크롤러 허용 (15) — low variance in KR dental, but a hard block is fatal.
  {
    ...
    b.push(item('crawl', 'AI 크롤러 허용', pts, 15, status, note, fix, 'AI 크롤러가 막히면 인용 자체가 불가능합니다.'));
  }
  ...
  // 2. 구조화 데이터 JSON-LD (20)
  {
    ...
    const fix = !j.hasDental
      ...
      : !j.hasFaq
        ...
        : null;
    b.push(item('schema', '구조화 데이터(JSON-LD)', pts, 20, status, note, fix, 'JSON-LD는 상호·진료·FAQ를 AI가 기계적으로 읽게 해줍니다.'));
  }
  ...
  // 3. E-E-A-T (15)
  {
    ...
    b.push(item('eeat', 'E-E-A-T(작성자·자격)', pts, 15, status, note, fix, '의료(YMYL) 주제는 작성자 자격이 있어야 AI가 신뢰합니다.'));
  }
  ...
  // 4. 신선도 (10)
  {
    ...
    b.push(item('fresh', '신선도', pts, 10, status, note, fix, '게시·수정일이 보이면 AI가 최신 정보로 우선합니다.'));
  }
  ...
  // 5. Answer-first 구조 (20)
  {
    ...
    b.push(item('answer', 'Answer-first 구조', pts, 20, status, note, fix, '질문에 결론부터 답하는 구조라야 AI가 그대로 인용합니다.'));
  }
  ...
  // 6. 추출성 S/N (10)
  {
    ...
    b.push(item('extract', '추출성(S/N)', pts, 10, status, note, fix, '본문 텍스트가 충분해야 AI가 추출·인용할 내용이 있습니다.'));
  }
  ...
  // 7. 엔티티·로컬 정합 (10)
  {
    ...
    b.push(item('local', '엔티티·로컬 정합', pts, 10, status, note, fix, '상호·주소·전화가 일치해야 지역 질의에서 엔티티로 잡힙니다.'));
  }
  ...
  // gain = HYGIENE points only. NOT a prediction of more citations (자사 검증: 점수↔인용 무상관).
  // gainLabel is carried so the UI can never render "+N" as a citation/exposure promise.
  const topFixes = b
    .filter((x) => x.fix)
    .sort((a, c) => c.max - c.points - (a.max - a.points))
    .slice(0, 3)
    .map((x) => ({ label: x.label, fix: x.fix, gain: x.max - x.points, gainLabel: '구조 위생 점수 · 인용 예측 아님' }));
...
function item(key, label, points, max, status, note, fix, why) {
  return { key, label, points, max, status, note, fix, why };
}
