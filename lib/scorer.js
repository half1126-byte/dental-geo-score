...
export function scorePage({ robots = {}, signals = {} }) {
  ...
  // 2. 구조화 데이터 JSON-LD (20)
  {
    ...
    if (j.hasFaq) pts += 5;
    if (j.hasTrust) pts += 5;
    // KDD 2024 / Auriti GEO: sameAs authority links +27% citation lift
  ...
  // 5. Answer-first 구조 (20)
  {
    ...
    // KDD 2024: statistics +33%, quotations +41% citation lift
    if (s.hasStatistics) pts += 2;
    if (s.hasQuotations) pts += 2;
    pts = Math.min(20, pts);
    const status = pts >= 14 ? 'ok' : pts > 0 ? 'warn' : 'fail';
    const note = `질문형 H2 ${s.questionH2 || 0}개, 표 ${s.tables || 0}개, FAQ블록 ${s.faqBlocks || 0}개${s.hasStatistics ? ' · 통계O' : ''}${s.hasQuotations ? ' · 인용구O' : ''}`;
  ...
  const topFixes = b
    ...
    .slice(0, 3)
    .map((x) => ({ label: x.label, fix: x.fix, gain: x.max - x.points, gainLabel: '구조 위생 점수 · 인용 예측 아님', note: x.why }));
