...
const SIGNAL_WHY = {
  ...
  citable: '출처·인용구·수치가 붙은 문단이 답변에 그대로 발췌되기 쉽습니다(KDD 2024 실측 상위 3개 기법).',
  extract: '본문 가시 텍스트 비중이 충분해야 추출할 내용이 존재합니다.',
  local: '상호·주소·전화(NAP)가 일치해야 지역 질의에서 엔티티로 잡힙니다.',
};

const SIGNAL_DIFFICULTY = {
  crawl: '하', schema: '중', eeat: '중', fresh: '하', answer: '상', citable: '중', extract: '중', local: '하',
};
...
const GAP_PRODUCTS = {
  ...
  fresh: ['blog-posting'],
  answer: ['content-hub'],
  citable: ['blog-posting', 'content-hub'],
  extract: ['content-hub'],
...
export function buildSignalDiff(userBreakdown, compBreakdown) {
  const order = ['crawl', 'schema', 'eeat', 'citable', 'answer', 'fresh', 'extract', 'local'];
  const rows = [];
  for (const key of order) {
    const u = pick(userBreakdown, key);
    const c = pick(compBreakdown, key);
    if (!u && !c) continue;
    // 측정 제한(JS 렌더로 못 읽은 신호)은 '없음'이 아니다 → 격차로 단정하지 않고 '—'로 둔다.
    const uLimited = !!(u && u.measurement === 'limited');
    const uOk = u && u.status === 'ok';
    const cOk = c && c.status === 'ok';
    let verdict = 'same';
    if (uLimited) verdict = 'same';
    else if (!uOk && cOk) verdict = 'gap';
    ...
    rows.push({
      ...
      layer: (u || c).layer || '',
      weight: (u || c).max || 0,
      userStatus: uLimited ? 'na' : (u ? u.status : 'na'),
