...
test('lead: 400 when clinicName missing', async () => {
  const origKey = process.env.OPERATOR_KEY;
  process.env.OPERATOR_KEY = 'test-key';
  const req = mockReq(
    { contactName: '홍길동', phone: '010-1234-5678', selectedProducts: ['geo-diagnosis'] },
    { 'x-operator-key': 'test-key' }
  );
  const res = mockRes();
  await handler(req, res);
...
test('lead: 400 when selectedProducts empty array', async () => {
  ...
  process.env.OPERATOR_KEY = 'test-key';
  const req = mockReq(
    { clinicName: '이도치과', contactName: '홍길동', phone: '010-1234-5678', selectedProducts: [] },
    { 'x-operator-key': 'test-key' }
  );
  const res = mockRes();
  await handler(req, res);
...
test('lead: 400 when phone missing', async () => {
  ...
  process.env.OPERATOR_KEY = 'test-key';
  const req = mockReq(
    { clinicName: '이도치과', contactName: '홍길동', selectedProducts: ['geo-diagnosis'] },
    { 'x-operator-key': 'test-key' }
  );
  const res = mockRes();
  await handler(req, res);
